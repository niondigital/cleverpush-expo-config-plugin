...
const withCleverPush: ConfigPlugin<CleverPushPluginProps> = (config, props) => {
	...
	config = withExtensionInXcodeProject(config, { targetName: NSE_TARGET_NAME, files: NSE_FILES, sourceFile: NSE_SOURCE_FILE });
	...
	if (props.includeContentExtension) {
		...
		config = withExtensionInXcodeProject(config, { targetName: NCE_TARGET_NAME, files: NCE_FILES, sourceFile: NCE_SOURCE_FILE });
	}
